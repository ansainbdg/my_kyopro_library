...
def test_code(session, code, input_txt, LanguageId = 5078):#pypy
...
def execute_case_interactive(args, file="code1.py", inputdir='in', outputdir='out', TL=None):
    ...
    try:
        ...
        proc = subprocess.run(
            f'cargo run --release --bin tester {command} < {input_file_path} > {output_file_path}', stdout=subprocess.PIPE ,stderr=subprocess.PIPE, shell=True, timeout=TL)
...
def execute_case(args, file="code1.py", inputdir='in', outputdir='out', TL=None):
    ...
    with open(input_file_path) as fin:
        with open(output_file_path, 'w') as fout:
            ...
            try:
                proc = subprocess.run(
                    command, stdin=fin, stdout=fout, stderr=subprocess.PIPE, timeout=TL)
            except subprocess.TimeoutExpired:
                return seed, 0, 0,"TimeoutExpired",""
            end_time = time.perf_counter()
            try:
...
def execute_case_atcoder(args, file="code1.py", inputdir='in', outputdir='out'):
    ...
    login(session)
    stdout, stderr, TimeConsumption = test_code(session, code, input_text, LanguageId)
...
def execute_case_main(args,file="code1.py",interactive=False,atcoder=False,inputdir='in', outputdir='out'):
    if file[-4:]==".cpp":
        file_path = f"{file[:-4]}.exe"
        if os.path.isfile(file_path):
            os.remove(file_path)
        proc = subprocess.run(["g++","-o",file[:-4],file],stderr=subprocess.PIPE)
        if not os.path.isfile(file_path):
            print(f"result: CompileError")
            return re.split(r'\r\n|\n|\r|\x85|\u2028|\u2029', proc.stderr.decode("utf-8"))
...
def main(CASE=100, file='code1.py', interactive=False, to_csv=False, use_koteiseed=False, multi=False, print_score=True, TL=None, **option):
    ...
    if file[-4:]==".cpp":
        ...
        proc = subprocess.run(["g++","-o",file[:-4],file],stderr=subprocess.PIPE)
        if not os.path.isfile(file_path):
            raise SyntaxError(proc.stderr.decode("utf-8"))
    optionstr = ''
    indir = 'in'
    outdir = 'out'
